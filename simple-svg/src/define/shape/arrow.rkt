  ...
  (let* (
    ...
    (format "    <polygon id=\"~a\"\n~a"
            ...
            (with-output-to-string
              (lambda ()
                (printf "          points=\"\n")
                (printf "            ~a,~a\n" (svg-precision (car handle_bottom_left)) (svg-precision (cdr handle_bottom_left)))
                (printf "            ~a,~a\n" (svg-precision (car handle_bottom_right)) (svg-precision (cdr handle_bottom_right)))
                (printf "            ~a,~a\n" (svg-precision (car Q)) (svg-precision (cdr Q)))
                (printf "            ~a,~a\n" (svg-precision (car R)) (svg-precision (cdr R)))
                (printf "            ~a,~a\n" (svg-precision (car S)) (svg-precision (cdr S)))
                (printf "            ~a,~a\n" (svg-precision (car handle_top_right)) (svg-precision (cdr handle_top_right)))
                (printf "            ~a,~a\n" (svg-precision (car handle_top_left)) (svg-precision (cdr handle_top_left)))
                (printf "            \"/>\n")
                )))))
